...
function saveState() {
  ...
  try {
    ...
    const myId = tg?.initDataUnsafe?.user?.id || 'demo_user';
    localStorage.setItem(`${STORAGE_KEY}_user_${myId}`, JSON.stringify(state));
  } catch (err) {
...
function handleReferral(inviterId) {
  ...
  // PLACEHOLDER: In production, this call goes to your backend, which:
  //  1. Verifies initData & inviterId
  //  2. Credits inviterId's account with REFERRAL_BONUS coins
  //  3. Adds this player to inviterId's friends list
  console.log(`[Referral Placeholder] Notify backend: inviter=${inviterId} should receive +${REFERRAL_BONUS} coins for referring user=${myId}`);

  // Local demo simulation: if the inviter's save data exists in THIS browser
  // (e.g. testing both roles on the same device), credit them directly.
  try {
    const inviterRaw = localStorage.getItem(`${STORAGE_KEY}_user_${inviterId}`);
    if (inviterRaw) {
      const inviterState = JSON.parse(inviterRaw);
      inviterState.coins = (inviterState.coins || 0) + REFERRAL_BONUS;
      inviterState.friends = inviterState.friends || [];
      inviterState.friends.push(state.username || `User ${myId}`);
      localStorage.setItem(`${STORAGE_KEY}_user_${inviterId}`, JSON.stringify(inviterState));
    }
  } catch (err) {
    console.error('Referral demo sync failed:', err);
  }
}
